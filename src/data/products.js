// Configuración de productos
// Agrega aquí todos los productos con sus imágenes y datos

export const products = [
  // Navidad — descomentar para próxima temporada
  /* {
    id: 1,
    nombre: "Centro de Mesa Navideño",
    descripcion: "Transforma tu mesa en el corazón de las fiestas con este elegante centro de mesa navideño. Fabricado con base de madera natural y velón de soja artesanal, disponible en diferentes fragancias que llenarán tu hogar de magia navideña. Perfecto para crear un ambiente acogedor y festivo durante las celebraciones.",
    precio: "15000",
    categoria: "centros",
    imagen: "/assets/1.png",
    whatsapp: "5491122714366"
  }, */
  /* {
    id: 2,
    nombre: "Centro de Mesa en Copas",
    descripcion: "Añade elegancia navideña a tu decoración con este hermoso centro de mesa en copas de agua. Cada pieza incluye un velón de soja artesanal con diferentes fragancias disponibles. Ideal para crear un ambiente sofisticado y festivo. Precio $15.000 c/u. Combo especial: dos copas $25.000.",
    precio: "15000",
    categoria: "centros",
    imagen: "/assets/2.jpg",
    whatsapp: "5491122714366"
  }, */
  /* {
    id: 3,
    nombre: "Vela de Soja con Papá Noel",
    descripcion: "Dale vida a tu decoración navideña con estas encantadoras velas de soja decoradas con motivos de Papá Noel. Disponibles en color verde o rojo, cada vela contiene 160 gramos de cera de soja 100% natural con aromas variados. Perfectas para crear un ambiente mágico y acogedor durante las fiestas.",
    precio: "9000",
    categoria: "velas",
    imagen: "/assets/3.jpg",
    whatsapp: "5491122714366"
  }, */
  {
    id: 4,
    nombre: "Set Decorativo Elefante",
    descripcion: "Un set decorativo único que combina arte y funcionalidad. Incluye una vela de soja artesanal en forma de elefante, acompañada de una maceta y bandeja coordinadas. Disponible en diferentes colores y fragancias para que elijas el que mejor combine con tu estilo. Ideal como regalo especial o para decorar cualquier rincón de tu hogar.",
    precio: "15000",
    categoria: "sets",
    imagen: "/assets/4.png",
    whatsapp: "5491122714366"
  },
  /* {
    id: 5,
    nombre: "Velas en Copas de Sidra",
    descripcion: "Combina tradición y elegancia con estas hermosas velas en copas de sidra. Elaboradas con cera de soja artesanal, cada pieza presenta distintas decoraciones únicas y está disponible en varios aromas. Perfectas para crear un ambiente cálido y sofisticado en cualquier espacio de tu hogar.",
    precio: "15000",
    categoria: "velas",
    imagen: "/assets/5.png",
    whatsapp: "5491122714366"
  }, */
  /* {
    id: 6,
    nombre: "Velón Venado",
    descripcion: "Añade un toque natural y elegante a tu decoración con estos encantadores velones en forma de venado. Cada pieza contiene 160 gramos de cera de soja artesanal, disponible en diferentes colores y aromas. Con una duración excepcional de entre 13 y 24 horas, son perfectos para crear un ambiente acogedor y relajante.",
    precio: "9500",
    categoria: "velas",
    imagen: "/assets/6.png",
    whatsapp: "5491122714366"
  }, */
  /* {
    id: 7,
    nombre: "Centro de Mesa Pino",
    descripcion: "Crea el ambiente navideño perfecto con este hermoso centro de mesa que combina un velón en forma de pino con una base de madera decorada. Disponible en diferentes aromas navideños, cada pieza ofrece una duración excepcional de entre 13 y 24 horas. Ideal para convertir tu mesa en el punto focal de las celebraciones.",
    precio: "15000",
    categoria: "centros",
    imagen: "/assets/7.png",
    whatsapp: "5491122714366"
  }, */
  {
    id: 8,
    nombre: "Vela Alhajero",
    descripcion: "Una pieza única que combina funcionalidad y elegancia. Esta vela de soja artesanal viene en un envase con forma de alhajero, disponible en diferentes colores y aromas. Perfecta para decorar cualquier espacio mientras llena el ambiente con fragancias deliciosas. Ideal como regalo especial o para tu propia colección.",
    precio: "9700",
    categoria: "velas",
    imagen: "/assets/8.png",
    whatsapp: "5491122714366"
  },
  {
    id: 9,
    nombre: "Set Figuras Abstractas en Yeso + Bandeja",
    descripcion: "Dale un toque moderno a tus espacios con este set decorativo de tres figuras abstractas en yeso, color negro mate. Vienen sobre una bandeja blanca que resalta el contraste y aporta elegancia. Ideal para livings, escritorios, estanterías o como regalo especial.",
    precio: "7500",
    categoria: "sets",
    imagen: "/assets/9.png",
    whatsapp: "5491122714366",
    textura: "marmol"
  },
  {
    id: 11,
    nombre: "Vaso Plateado",
    descripcion: "Elegancia y sofisticación en un solo producto. Este vaso plateado contiene una vela de soja artesanal de 200 gramos, disponible en varios aromas exquisitos. Con una duración impresionante de 14 a 24 horas, es perfecta para crear un ambiente lujoso y acogedor. Ideal para ocasiones especiales o para disfrutar en tu día a día.",
    precio: "20000",
    categoria: "velas",
    imagen: "/assets/11.png",
    whatsapp: "5491122714366"
  },
  {
    id: 12,
    nombre: "Velón Burbujas",
    descripcion: "Un diseño moderno y único que captura la atención. Este velón con efecto burbujas contiene 150 gramos de cera de soja artesanal, disponible en varias combinaciones de colores y aromas. Con una duración de entre 13 y 24 horas, es perfecto para añadir un toque contemporáneo y elegante a cualquier espacio de tu hogar.",
    precio: "8500",
    categoria: "velas",
    imagen: "/assets/12.png",
    whatsapp: "5491122714366"
  },
  {
    id: 13,
    nombre: "Vela Vaso Vidrio Rústica",
    descripcion: "Aporta calidez y estilo rústico a tu decoración con estas hermosas velas en vaso de vidrio. Cada pieza contiene 150 gramos de cera de soja artesanal, disponible en diferentes aromas. Con una decoración rústica única y una duración de entre 13 y 24 horas, son perfectas para crear un ambiente acogedor y natural en cualquier rincón de tu hogar.",
    precio: "9500",
    categoria: "velas",
    imagen: "/assets/13.png",
    whatsapp: "5491122714366",
    textura: "lino"
  },
  /* {
    id: 15,
    nombre: "Centro de Mesa Árbol Navidad",
    descripcion: "El centro de mesa navideño perfecto para tu hogar. Combina una base de madera elegante con un velón en forma de árbol de Navidad de 120 gramos. Disponible en diferentes aromas y colores, cada pieza ofrece una duración de 12 a 18 horas. Transforma tu mesa en el corazón de las celebraciones navideñas con este hermoso diseño.",
    precio: "15000",
    categoria: "centros",
    imagen: "/assets/15.jpg",
    whatsapp: "5491122714366"
  }, */
  /* {
    id: 16,
    nombre: "Centro de Mesa en Copas de Agua",
    descripcion: "Añade elegancia navideña a tu mesa con este hermoso centro de mesa en copas de agua. Cada pieza incluye un velón de soja artesanal con decoración navideña, disponible en varios aromas deliciosos. Con una duración de 12 horas, es perfecto para crear un ambiente festivo y acogedor durante las celebraciones.",
    precio: "15000",
    categoria: "centros",
    imagen: "/assets/16.png",
    whatsapp: "5491122714366"
  }, */
  {
    id: 17,
    nombre: "Vela Violeta - Pimienta Naranja",
    descripcion: "Vela artesanal elaborada con cera de soja 100% natural. Su aroma Pimienta Naranja combina notas cítricas frescas con un toque especiado que aporta energía y calidez al ambiente. Ofrece una combustión limpia, uniforme y de larga duración. Ideal para aromatizar y decorar cualquier espacio. Contenido: 170 g.",
    precio: "8500",
    categoria: "velas",
    imagen: "/assets/17.png",
    whatsapp: "5491122714366"
  },
  {
    id: 18,
    nombre: "Flor de Loto de Yeso",
    descripcion: "Una pieza única que combina arte y espiritualidad. Esta hermosa flor de loto elaborada en yeso incluye una vela de soja artesanal, disponible en diferentes aromas y colores. Perfecta para crear un ambiente sereno y elegante, ideal para espacios de meditación, dormitorios o como elemento decorativo especial en cualquier rincón de tu hogar.",
    precio: "5500",
    categoria: "velas",
    imagen: "/assets/18.png",
    whatsapp: "5491122714366"
  },
  {
    id: 19,
    nombre: "Velas de soja decorativas – Búhos",
    descripcion:
      "Dulces y delicadas velas artesanales en forma de búho, ideales para decorar cualquier espacio con un toque cálido y original. Elaboradas a mano, con detalles texturizados y terminaciones pintadas que realzan su diseño único. Perfectas para regalos, decoración del hogar o souvenirs especiales. Material: cera de soja, hechas artesanalmente, con colores suaves y elegantes.",
    precio: "12000",
    categoria: "velas",
    imagen: "/assets/19.jpeg",
    whatsapp: "5491122714366"
  },
  {
    id: 20,
    nombre: "Maceta rústica artesanal",
    descripcion:
      "Delicada pieza de estilo rústico, realizada a mano con terminación suave y detalles sutiles que transmiten calidez y armonía. Su diseño tierno y minimalista la convierte en el complemento ideal para decorar espacios con un toque natural y acogedor. Ideal para ambientar rincones especiales, acompañar velas o sumar a composiciones con madera y flores secas. Cada pieza es única. Precios: maceta sola $3.000 · con velas aromáticas para hornito $6.500.",
    categoria: "sets",
    imagen: "/assets/20.jpeg",
    whatsapp: "5491122714366"
  },
  {
    id: 21,
    nombre: "Velas en vaso con sales del Himalaya",
    descripcion:
      "Velas artesanales en vaso de vidrio, elaboradas con cera de alta calidad y delicadamente decoradas con sales del Himalaya en su interior. Su diseño en capas crea un efecto visual único, ideal para ambientar espacios con calidez y armonía. Perfectas para regalar o acompañar momentos de relax.",
    precio: "12000",
    categoria: "velas",
    imagen: "/assets/21.jpeg",
    whatsapp: "5491122714366"
  },
  {
    id: 22,
    nombre: "Velas en cuencos de cerámica con figuras decorativas",
    descripcion:
      "Velas artesanales en delicados cuencos de cerámica blanca, rellenas con pequeñas figuras decorativas en cera color rosa. Cada pieza combina formas encantadoras —botones, frutas y figuras suaves— creando un diseño único y visualmente atractivo. Ideales para regalar o decorar con un toque dulce y elegante; aportan calidez y estilo en ocasiones especiales o en el día a día del hogar. Hechas a mano, con diseño original y presentación cuidada.",
    precio: "7500",
    categoria: "velas",
    imagen: "/assets/22.jpeg",
    whatsapp: "5491122714366"
  },
  {
    id: 23,
    nombre: "Velas en vaso con flores en cera y base mármol",
    descripcion:
      "Velas artesanales en vaso de vidrio, elaboradas con cera blanca y decoradas con delicadas figuras en forma de flor en tono rosa, realzadas con un sutil toque de brillo. Presentadas sobre base tipo mármol, transmiten una estética limpia, elegante y moderna, ideal para ambientar con calidez y estilo. Perfectas para regalos, eventos especiales o decoración del hogar: combinan elaboración artesanal y armonía visual en una pieza única.",
    precio: "9500",
    categoria: "velas",
    imagen: "/assets/23.jpeg",
    whatsapp: "5491122714366"
  },
  {
    id: 24,
    nombre: "Vela artesanal decorativa",
    descripcion:
      "Pieza delicada y elegante para transformar cualquier espacio en un ambiente cálido y armonioso. Presentada en vaso de vidrio transparente, con detalles en tonos suaves y acabado minimalista: combina estética cuidada y elaboración artesanal. Ideal para decorar, regalar o acompañar momentos de relajación, con un toque moderno y sofisticado.",
    precio: "11000",
    categoria: "velas",
    imagen: "/assets/24.jpeg",
    whatsapp: "5491122714366"
  },
  {
    id: 25,
    nombre: "Vela degradé verde en vaso",
    descripcion:
      "Vela artesanal en vaso de vidrio con delicado degradé verde y terminación minimalista. Pensada para ambientes cálidos, relajantes y sofisticados; decora con un toque natural y elegante. Diseño moderno que une armonía visual y calidez: ideal para regalar o disfrutar en momentos de relax. Hecha artesanalmente, con estética premium y natural; apta para decoración y aromaterapia.",
    precio: "11000",
    categoria: "velas",
    imagen: "/assets/25.jpeg",
    whatsapp: "5491122714366"
  },
  {
    id: 26,
    nombre: "Vela degradé verde en vaso – formato amplio",
    descripcion:
      "Vela artesanal en vaso de vidrio con delicado degradé verde y terminación minimalista. Pensada para ambientes cálidos, relajantes y sofisticados; decora con un toque natural y elegante. Diseño moderno que une armonía visual y calidez: ideal para regalar o disfrutar en momentos de relax. Hecha artesanalmente, con estética premium y natural; apta para decoración y aromaterapia.",
    precio: "13000",
    categoria: "velas",
    imagen: "/assets/26.jpeg",
    whatsapp: "5491122714366"
  },
  {
    id: 27,
    nombre: "Vela de soja Pomelo Kiwi con flores secas",
    descripcion:
      "Vela artesanal de soja con flores secas naturales y aroma Pomelo Kiwi: combinación fresca, cítrica y frutal que aporta energía y bienestar. Notas vibrantes de pomelo y la dulzura del kiwi generan una fragancia moderna, suave y refrescante. Decorada con flores secas para un estilo elegante, rústico y natural: ideal para armonizar espacios, regalar o momentos de relax.",
    precio: "15000",
    categoria: "velas",
    imagen: "/assets/27.jpeg",
    whatsapp: "5491122714366"
  },
  {
    id: 28,
    nombre: "Vela de soja Pomelo Kiwi con flores secas – presentación mediana",
    descripcion:
      "Vela artesanal de soja con flores secas naturales y aroma Pomelo Kiwi: combinación fresca, cítrica y frutal que aporta energía y bienestar. Notas vibrantes de pomelo y la dulzura del kiwi generan una fragancia moderna, suave y refrescante. Decorada con flores secas para un estilo elegante, rústico y natural: ideal para armonizar espacios, regalar o momentos de relax.",
    precio: "13000",
    categoria: "velas",
    imagen: "/assets/28.jpeg",
    whatsapp: "5491122714366"
  },
  {
    id: 29,
    nombre: "Vela artesanal multicapa",
    descripcion:
      "Combinación delicada de colores suaves y armoniosos que transforma cualquier espacio en un rincón de calma y bienestar. Elaborada artesanalmente con cera de soja, con combustión limpia y experiencia cálida y relajante. Diseño en capas pastel, aroma suave y envolvente. Ideal para decorar, regalar o acompañar momentos de descanso y conexión.",
    precio: "15000",
    categoria: "velas",
    imagen: "/assets/29.jpeg",
    whatsapp: "5491122714366"
  },
  {
    id: 30,
    nombre: "Vela artesanal decorativa con piedras multicolor",
    descripcion:
      "Elegancia, calidez y color en una sola pieza. Vela artesanal en vaso de vidrio con diseño minimalista y una delicada franja de piedras decorativas multicolor, moderna y sofisticada. Ideal para livings, dormitorios, espacios de relajación u ocasiones especiales. Hecha artesanalmente, con cera de calidad y diseño decorativo exclusivo.",
    precio: "11000",
    categoria: "velas",
    imagen: "/assets/30.jpeg",
    whatsapp: "5491122714366"
  },
  {
    id: 31,
    nombre: "Vela marmolada verde esmeralda y blanco",
    descripcion:
      "Elegancia y calidez en una pieza única hecha a mano. Combina tonos verde esmeralda y blanco con un delicado efecto marmolado, estilo moderno, natural y sofisticado. Ideal para decorar, crear ambientes relajantes o regalar un detalle especial. Diseño artesanal minimalista y premium, presentación en vaso de vidrio reutilizable; perfecta para decoración y aromaterapia.",
    precio: "12000",
    categoria: "velas",
    imagen: "/assets/31.jpeg",
    whatsapp: "5491122714366"
  },
  {
    id: 32,
    nombre: "Vela de soja aroma Sándalo Dulce con sales del Himalaya",
    descripcion:
      "Vela artesanal pensada para transformar el espacio en un ambiente cálido, relajante y sofisticado. El aroma a sándalo dulce aporta notas suaves, amaderadas y envolventes, ideales para momentos de calma. Decorada con sales del Himalaya: estética minimalista y toque natural. Cera de soja 100% artesanal. Ideal para decorar, regalar o rituales de relajación.",
    precio: "15000",
    categoria: "velas",
    imagen: "/assets/32.jpeg",
    whatsapp: "5491122714366"
  },
  {
    id: 33,
    nombre: "Vela geoda artesanal",
    descripcion:
      "Pieza única inspirada en la belleza de los cristales naturales. Elaborada artesanalmente con cera de soja, combina tonos violetas y rosas con destellos que realzan un diseño sofisticado. Estética tipo geoda, elegante y moderna. Ideal para decorar, regalar o acompañar momentos de relajación y armonía.",
    precio: "5500",
    categoria: "velas",
    imagen: "/assets/33.jpeg",
    whatsapp: "5491122714366"
  },
  {
    id: 34,
    nombre: "Jabones artesanales",
    descripcion:
      "Belleza natural para tu piel. Hechos a mano con ingredientes seleccionados y delicados aceites esenciales. Aromas entre otros: lavanda, rosas, manzana verde; consultá por más fragancias y pedidos. Suaves, aromáticos y únicos.",
    precio: "6500",
    categoria: "jabones",
    imagen: "/assets/34.jpeg",
    whatsapp: "5491122714366"
  }
]
